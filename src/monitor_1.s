...
fill_ram:             ; pre-set 16 bytes of user RAM to zero
  sta BASE_ADDRESS,y
  iny
  cpy #$0f
  bne fill_ram
...
print_new_char:
  lda INKEY   ; the ASCII value stored at the beginning of the IRQ handler, ASCII comes from the get_keys function
  ; do something to convert the ASCII value to a binary and store in a buffer to build 1 or 2 byte binary words from keyboard input
  ; at the moment just print it on the LCD
  jsr print_char
